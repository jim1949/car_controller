...
class Kalman():
    def __init__(self):
        self.x_matix=np.matrix('0. 0. 0. 0.').T 
...
#don't know how to pick up the only element here.
    # plt.scatter(max(kalmanx[-1]),max(kalmany[-1]),s=1,c='red',alpha=0.5)
    # plt.scatter(observed_x,observed_y,s=1,c='blue',alpha=0.5)
    print(kalmanx[-1])

    return x,P,kalmanx[-1],kalmany[-1],result
...
def start():
    ...
    # detectarea=detect_area(-10.0,10.0,-3.0,-6.0)
        ...
        if path_plan.waitmessage>=11:
            ...
            if peoplestates.point_no!=0:
                ...
                #5)posterior estimation
                    ...
                    kalman.x_matix,kalman.P,kalmanx,kalmany,result=demo_kalman_xy(peoplestates.world_position_x_mean,peoplestates.world_position_y_mean,kalman.x_matix,kalman.P,kalman.result)
                    estimation.k_pose_x=max(kalmanx)
                    estimation.k_pose_y=max(kalmany)
